...
from tccs.forms import RegisterCustomerForm, RegisterEmployeeForm, LoginCustomerForm, LoginEmployeeForm, ConsignmentForm, TruckForm, BranchQueryForm, ApproveTruckForm, DispatchTruckForm, ReceiveTruckForm, ApproveIncomingTruckForm, ForgotPasswordForm_email, ForgotPasswordForm_password, TruckStatusForm
from tccs import db
...
def driver_truck_receive_page(token):
    ...
    if request.method == "POST":
        ...
        if received_truck_object:
            ...
            flash(
                f"TRUCK {received_truck_object.id} of volume {received_truck_object.volumeConsumed} cubic meters is enroute", category="success")
        else:
            flash(
                f"Consignment {received_truck_object.id} of volume {received_truck_object.volumeConsumed} cubic meters is not enroute", category="danger")
        return redirect(url_for('dashboard_page'))
...
@app.route('/google_maps_routes/', methods=["GET", "POST"])
def google_maps_routes_page():
    return render_template('google_maps_routes.html')
